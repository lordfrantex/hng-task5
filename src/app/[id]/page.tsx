...
const Page = ({ params }: { params: { id: string } }) => {
    ...
    const [fetchUser, setFetchUser] = useState<UserState | undefined>({})
    ...
    const mapIt = fetchLinks.map((link: LinkType) => {
        return {
            ...link,
    ...
    const mapLinks = (fetchLinks && fetchLinks.length > 0) && mapIt.map((link: LinkType & { color: string }) => (
        ...
        <div className=" relative z-50">
            ...
            <div className="  rounded-b-[32px] p-6 ">
                ...
                <div className=" py-4 sm:px-6 sm:bg-white flex justify-between gap-4 items-center rounded-xl mb-[102px]">
                    {
                        fetchUser?.email === fetchUser?.email && <Link href='/editor' className='rounded-lg text-custom_blue border border-custom_blue py-3 font-semibold px-7 whitespace-nowrap'>Back to Editor</Link>
                ...
                <div className="md:w-[349px] py-12 px-3 md:px-14 m-auto rounded-3xl md:bg-white z-10">
                    <div className="grid place-items-center mb-14">
                        ...
                        {fetchUser?.firstName ?
                            <h3 className="font-bold text-custom_dark text-[28px] mb-2">{fetchUser.firstName} {fetchUser.lastName}</h3>
